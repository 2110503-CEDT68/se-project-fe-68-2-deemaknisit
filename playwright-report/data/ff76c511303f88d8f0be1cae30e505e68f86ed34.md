...
- Name: us1-1-write-review.spec.ts >> US1-1: Write a review for a completed booking >> shows the Review button only for completed bookings
- Location: e2e/us1-1-write-review.spec.ts:17:7
...
Locator: getByRole('button', { name: /^Review$/i }).first()
Expected: visible
Timeout: 5000ms
...
```ts
  ...
  17  |   test('shows the Review button only for completed bookings', async ({ page }) => {
  18  |     await mockNextAuthSession(page);
  19  |     await mockReviewBackend(page, {
  20  |       bookings: [COMPLETED_BOOKING, PENDING_BOOKING],
  21  |       reviews: [],
  22  |     });
  23  | 
  24  |     await page.goto('/bookings');
  25  | 
  26  |     // Completed booking shows a Review button
  27  |     await expect(
  28  |       page.getByRole('button', { name: /^Review$/i }).first()
> 29  |     ).toBeVisible();
      |       ^ Error: expect(locator).toBeVisible() failed
  30  | 
